...
function instantiateExpansion(recipe: Recipe, modelId: string, cfg: AIGatewayConfig): any {
  switch (recipe.implementation) {
    ...
    case 'native-anthropic': {
      ...
      // 2026-07-28: honor provider_base_urls.anthropic so a local subscription
      // shim (Claude Code OAuth passthrough) can front api.anthropic.com.
      const anthropicBaseURL = cfg.base_urls?.['anthropic'];
      return createAnthropic({ apiKey, ...(anthropicBaseURL ? { baseURL: anthropicBaseURL } : {}) }).languageModel(modelId);
    }
    case 'openai-compatible': {
      // D12=A: unified auth via Recipe.resolveAuth (or default).
