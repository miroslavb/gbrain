...
export async function readPolicyOpts(
  ...
  // Fork (world-only host, migration v147 posture): the pre-seal chunk
  // withhold follows the SAME resolver as the private-page gate. Under
  // `facts.default_visibility=world` every agent caller on this host reads
  // the world view the trusted local CLI reads, so old-but-world chunks are
  // served instead of being withheld until a whole-brain reindex. Legacy /
  // multi-principal brains keep upstream's fail-closed `remote !== false`.
