 ...
 */
export function collectLexiconMentions(turns: WindowTurn[]): Map<string, LexiconMention> {
...
/**
 * Fork (2026-08-05) — admit all-lowercase mentions of KNOWN aliases as
 * candidates (see collectLexiconMentions), in place, before the stage's
 * no-candidates early return: an all-lowercase window has zero Capitalized
 * candidates yet may still name a seeded entity. Fail-open — a resolver error
 * (e.g. a brain without page_aliases) degrades to the Capitalized-only
 * behavior. Norms already covered by the Capitalized extractor are skipped.
 */
export async function admitLexiconCandidates(
  engine: Pick<BrainEngine, 'resolveAliases'>,
  turns: WindowTurn[],
  candidates: WindowEntityCandidate[],
  sourceIds: readonly string[],
): Promise<number> {
  const lexicon = collectLexiconMentions(turns);
  for (const norm of candidatesByNorm(candidates).keys()) lexicon.delete(norm);
  if (!lexicon.size) return 0;
  const probeNorms = Array.from(lexicon.keys());
  const probes = await Promise.allSettled(
    sourceIds.map((src) => engine.resolveAliases(probeNorms, { sourceId: src })),
  );
...
export async function volunteerContext(
  ...
  // Fork lexicon arm: lowercase mentions of KNOWN aliases join the pool
  // (DB membership decides; fail-open). Must run before volunteerStage's
  // no-candidates early return.
  await admitLexiconCandidates(engine, turns, candidates, opts.sourceIds);
