...
/** Commit intent before a best-effort DB-body mirror; never inferred from TTL. */
export async function recordFactWithdrawal(engine: BrainEngine, id: number, sourceId: string, worldOnly = false): Promise<void> {
  await engine.transaction(async tx => {
    await recordFactWithdrawalInTransaction(tx, id, sourceId, worldOnly);
  });
}
...
export async function recordFactWithdrawalInTransaction(
  tx: Pick<BrainEngine, 'executeRaw'>, id: number, sourceId: string, worldOnly = false,
): Promise<void> {
    ...
    const rows = await tx.executeRaw<{ visibility: string; fact: string; expired_at: unknown }>(
      `SELECT visibility,fact,expired_at FROM facts WHERE id=$1 AND source_id=$2
        AND ($3::boolean=false OR visibility='world') FOR UPDATE`, [id, sourceId, worldOnly]);
    if (!rows.length) throw new Error('Fact changed before withdrawal; retry with current fact id');
    const row = rows[0];
    if (row.expired_at !== null) throw new Error('Fact expired before withdrawal; refresh current facts');
