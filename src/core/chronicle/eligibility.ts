...
export const CHRONICLE_RESCUE_SLUG_PREFIXES = ['meetings/', 'conversations/', 'cal/', 'calendar/'] as const;
const MIN_BODY_CHARS = 80;
// Live 24h audit (2026-08-21): every explicit-message-count conversation below
// 100 messages returned no_events, while the only useful conversation had 809
// messages. Missing metadata stays backward-compatible; meetings/calendar
// events are never subject to this gate.
...
export function isChronicleEligible(input: {
  ...
  const conversationShape = type === 'conversation' || slug.startsWith('conversations/');
  ...
  const slugOk = CHRONICLE_RESCUE_SLUG_PREFIXES.some((p) => slug.startsWith(p));
