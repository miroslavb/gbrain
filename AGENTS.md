...
- **Fact identity/provenance (host priority6):** qualified canonical IDs outrank
  ...
  the claim without entity attribution and preserves its source page; manual
  writes require an explicit ID. `entity.ambiguous` qualifies its legacy ranked
  card. The page backstop carries origin across queue/inline execution. Exact
  fence replay dedup runs under the page lock without embeddings, compares
  file/DB source/time/kind/context/session coordinates and returns IDs in input
  order. Events/commitments and supersession bypass this shortcut. Retain
  independent evidence; dimensions need a concrete comparison.
...
- **Duplicate-content signal:** doctor counts every same-source content-hash
  ...
  a warning; never add a broad prefix exemption to make the check green.
...
- **Embedding migration outage containment:** the stale drain must open a
  ...
  per page. llama.cpp physical-batch overflow is content-shaped even when its
  HTTP surface reports 500; classify its stable message before generic
  transient handling so it neither opens the provider circuit nor darkens all
  sibling chunks. Migration progress is page-based with an unknown total unless an
...
Fact candidate lookup on PostgreSQL and PGlite must rank inside the active source/entity
...
PGlite uses halfvec/HNSW; its vector query cast does not prevent ANN filtering.
Keep the embedded halfvec regression and run the PostgreSQL test through the
ordinary validated E2E DATABASE_URL or its dedicated test URL.
