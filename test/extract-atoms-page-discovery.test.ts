...
describe('local extract-atoms config knobs', () => {
  // Fork policy (62dc5aab3): `note`/`conversation` are broad catch-all types
  // and need an explicit `atom_extract: true` opt-in before the phase spends
  // budget on them. Upstream's caps tests predate that gate, so the seeds
  // below opt in rather than the gate being relaxed — it exists to stop a
  // federated brain burning its atom budget on code, skills and archives.
  test('page_discovery_budget caps discovery; max_source_chars truncates the prompt slice', async () => {
    ...
    await seedPage({ slug: 'note/knob-1', type: 'note', frontmatter: { atom_extract: true }, compiled_truth: 'z'.repeat(2000) });
    await seedPage({ slug: 'note/knob-2', type: 'note', frontmatter: { atom_extract: true }, compiled_truth: 'z'.repeat(2000) });
...
describe('local extract-atoms config knobs — invalid-value fallbacks', () => {
  ...
  test("max_source_chars: '499' (below the 500 floor) rejected — default leaves a 2000-char payload untruncated", async () => {
    ...
    await seedPage({ slug: 'note/floor-reject', type: 'note', frontmatter: { atom_extract: true }, compiled_truth: 'z'.repeat(2000) });
    const captured: string[] = [];
  ...
  test("max_source_chars: '500' (at the floor) accepted — payload sliced to 500", async () => {
    ...
    await seedPage({ slug: 'note/floor-accept', type: 'note', frontmatter: { atom_extract: true }, compiled_truth: 'z'.repeat(2000) });
    const captured: string[] = [];
    await runPhaseExtractAtoms(engine, { _transcripts: [], _chat: capturingChat(captured) as never });
    expect(captured.length).toBe(1);
    const body = captured[0].split('\n\n---\n\n')[1] ?? '';
