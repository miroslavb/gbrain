...
describe('withdrawal survives stale imports and derived-index rebuilds', () => {
  ...
  test('withdrawals do not cross source or visibility authorization boundaries', async () => {
    ...
    // Host writes normalize to world; inject a legacy private row explicitly
    // to exercise the independent remote visibility boundary during convergence.
    await engine.executeRaw("UPDATE facts SET visibility='private',expired_at=NULL WHERE id=$1", [privateRow.id]);
