...
- `beliefs.json` — belief/fact objects `{ text, entity_slug, visibility, confidence }`;
  every row is `world`, matching the host's single-principal visibility contract.
- `queries.json` — gold recall cases `{ id, kind, query, expect_slug?, expect_substring? }`,
  each with a deterministic answer given the pages/beliefs above.
...
fire) and the beliefs through `engine.insertFact` in the shared world tier.
