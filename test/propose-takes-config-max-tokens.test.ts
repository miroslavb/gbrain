...
function buildMockEngine(config: Record<string, string>): BrainEngine {
  // Fork gate: `cycle.propose_takes.enabled` is fail-closed (default false) —
  // the explicit steady-state containment decision that keeps the phase from
  // proposing takes across a 46k-page brain unattended. Upstream's #4494 tests
  // predate it, so the mock enables it here; per-test config still wins.
  const withGate: Record<string, string> = { 'cycle.propose_takes.enabled': 'true', ...config };
  return {
    kind: 'pglite',
    async getConfig(key: string): Promise<string | null> {
      return withGate[key] ?? null;
    },
