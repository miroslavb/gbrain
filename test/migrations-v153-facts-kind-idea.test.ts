/**
 * Migration v153 (facts_kind_idea_alter) — F5.
 *
 ...
 * and pre-v153 brains carry the 5-kind predicate: an INSERT with kind='idea'
 * violates it. v153 probes pg_constraint for the autogen name, drops it if
 ...
 *
 * Pinned contracts:
 * 1. v153 exists with the canonical name, idempotent flag, and a sql block
 *    carrying the probe + the widened 6-kind predicate.
 * 2. Fresh init: the facts kind CHECK admits 'idea'; an INSERT succeeds.
 * 3. Upgrade: on a brain restored to the pre-idea 5-kind CHECK (positive
 *    control: kind='idea' verifiably rejected), a ledger rewind to 152 →
 *    runMigrations swaps the constraint, the INSERT succeeds, re-run applies
 *    nothing, and re-executing the v153 SQL directly converges (drop+re-add
...
const V153_SQL = MIGRATIONS.find(m => m.version === 153)?.sql ?? '';
...
async function insertIdeaFact(fact: string): Promise<void> {
  await engine.executeRaw(
    `INSERT INTO facts (fact, kind, source) VALUES ($1, 'idea', 'test:v153')`,
    [fact],
  );
}

describe('migration v153 — structure', () => {
  test('exists with canonical name, idempotent flag, probe + widened predicate', () => {
    const v153 = MIGRATIONS.find(m => m.version === 153);
    expect(v153).toBeDefined();
    expect(v153?.name).toBe('facts_kind_idea_alter');
    expect(v153?.idempotent).toBe(true);
    expect(V153_SQL).toContain(`conname = 'facts_kind_check'`);
    expect(V153_SQL).toContain('DROP CONSTRAINT facts_kind_check');
    expect(V153_SQL).toContain(`CHECK (kind IN (${WIDENED_LIST}))`);
    expect(LATEST_VERSION).toBeGreaterThanOrEqual(153);
  });
});

describe('migration v153 — fresh init (PGLite)', () => {
...
describe('migration v153 — upgrade from a pre-v153 brain (PGLite)', () => {
  test('swaps the 5-kind constraint for the widened one; re-run applies nothing', async () => {
    // Simulate a pre-v153 brain: clear idea rows (they'd violate the narrow
    ...
    // Positive control: the pre-v153 shape verifiably rejects kind='idea'.
    await expect(insertIdeaFact('blocked idea row')).rejects.toThrow();

    // Apply v153 via the real migration runner (ledger rewind below 153).
    await engine.setConfig('version', '152');
