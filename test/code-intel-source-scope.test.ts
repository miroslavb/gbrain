...
describe('code reads require trusted local context while remote authorization is suspended', () => {
  for (const name of readOps) {
    test(`${name}: remote and omitted trust fail before accessing storage for every source grant`, async () => {
      // Fork (world-only host): the gate consults the brain's read posture
      // (`facts.default_visibility`) through the same resolver as the
      // private-page gate, so ONE config read is expected; storage itself
      // (pages/chunks/edges) must still never be touched before the refusal.
      let accesses = 0;
      const inaccessibleEngine = new Proxy({}, {
        get(_target, prop) {
