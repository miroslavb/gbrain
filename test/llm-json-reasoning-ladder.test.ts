...
describe('parseLlmJson — reasoning ladder', () => {
  ...
  test('recovers the ANSWER from a closed <thinking> block (sibling tag)', () => {
    // Some models/proxies emit <thinking> rather than <think>.
...
describe('extractors route through the ladder', () => {
  ...
  test('atoms reasoning retry preserves source grounding and the atomicity gate', () => {
    const quote = 'Acme Corp reduced invoice latency from five days to two days.';
    const sourceText = `${quote} ${'Supporting context. '.repeat(40)}`;
    const raw = '<think>draft: [{"title":"Draft"}]</think>' + JSON.stringify([{
      title: 'Invoice latency improvement',
      atom_type: 'insight',
      body: quote,
      source_quote: quote,
    }]);
