...
(databaseUrl ? test : test.skip)('HTTP delegation completes in a real worker with a bound tool and attributed spend', async () => {
  ...
  let priorVisibility: string | null | undefined;
  ...
  try {
    ...
    await withEnv({ GBRAIN_HOME: root, DATABASE_URL: undefined, GBRAIN_DATABASE_URL: undefined }, async () => {
      ...
      priorVisibility = await engine.getConfig('facts.default_visibility');
    ...
    if (engine) {
      try {
        ...
        if (priorVisibility === null) await engine.unsetConfig('facts.default_visibility');
        else if (priorVisibility !== undefined) await engine.setConfig('facts.default_visibility', priorVisibility);
