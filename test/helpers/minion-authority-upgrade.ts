...
export async function rehearseMinionAuthorityUpgrade(engine: BrainEngine): Promise<void> {
  ...
  // The deployed fork already owns versions through 154. The authority cutover
  // must still execute when upgrading that deployed ledger, not overwrite it.
  expect(MIGRATIONS.find(m => m.version === 155)?.name).toBe('oauth_client_capability_grants');
  expect(MIGRATIONS.find(m => m.version === 156)?.name).toBe('durable_fact_withdrawals');
  expect(migration.version).toBe(157);
  ...
  await engine.setConfig('version', '156');
  ...
  try {
    ...
    expect(await engine.getConfig('version')).toBe('156');
