...
describe('extract_rollup_7d.expected_limit_count (#4482)', () => {
  ...
  test('doctor: cap-only stops do NOT raise the failure warning; error halts still do', async () => {
    ...
    // Order matters against this fork's extract_health recovery latch
    // (51e7dcd51/0a7f6f302): a kind whose CURRENT state has since completed is
    // reported as recovered rather than warned, because a raw 7-day halt RATE
    // is a misleading health signal — a run that hits its budget ceiling books
    // a halt. So the completion is written first and the halt last, leaving the
    // current state halting. The rate is unchanged (1 halt / 1 completion) and
    // upstream's assertion — error halts still warn, cap-only stops do not —
    // holds exactly.
    await upsertExtractRollup(engine, {
