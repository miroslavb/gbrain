...
4. Back up the host migration ledger alongside the database and config plane
   (normally `~/.gbrain/migrations/completed.jsonl`), preserving mode, owner,
   and checksum. A database restore alone is not a complete migration snapshot:
   host migrations use this ledger to avoid replaying historical filesystem/
   fence work.
5. Beware: candidate CLI startup can auto-run pending schema migrations even
...
5. For a staging restore, seed its isolated `GBRAIN_HOME` with a verified copy
   of the production host migration ledger before running migrations. First
   prove the restored database's pre-migration schema/version, then migrate and
   repeat the migration command to prove idempotency. An empty ledger can make a
   valid restored DB replay historical host migrations and hit duplicate rows.
6. Run targeted regressions, generated-artifact parity, verify/typecheck/diff
   checks, the canonical full suite, and an adversarial read-only review.
7. Commit the exact green snapshot and create explicit old/candidate rollback
   refs before cutover.
...
4. Restore the verified host migration ledger into the effective production
   `GBRAIN_HOME` before migration. Read back both the database schema version and
   ledger identity; do not silently initialize an empty ledger against a mature
   database.
5. Apply migrations explicitly and non-interactively. Preserve custom supervisor
   topology with `--no-autopilot-install` when appropriate.
6. Start only GBrain HTTP/autopilot components. Keep the Hermes gateway parent
   alive.
