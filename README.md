...
If `claude` is not found, install Claude Code first — or use the per-harness blocks in the [protocol doc](docs/protocol/MEMORY_VERBS_v1.md). Memories use the single-principal host contract: `visibility: "world"` is the only supported value, so every connected agent can recall them.
...
- **`gbrain-base-v2`** (default as of v0.41.22) — the DRY/MECE canonical taxonomy, `note` as the catch-all: `analysis`, `atom`, `company`, `concept`, `conversation`, `deal`, `diary`, `email`, `event`, `extract_receipt`, `media`, `meeting`, `note`, `person`, `project`, `slack`, `social-digest`, `source`, `tweet`, `writing`. Subtypes/format/origin pushed to frontmatter. The taxonomy that responds to issue #1479. (The set grows with the pack — `gbrain schema stats` prints what your install actually declares.)
...
- **Voice**: Phone calls create brain pages via Twilio + OpenAI Realtime (or DIY STT+LLM+TTS). Install it with `gbrain integrations install agent-voice --target <repo>` ([`recipes/agent-voice.md`](recipes/agent-voice.md)); the older [`recipes/twilio-voice-brain.md`](recipes/twilio-voice-brain.md) is deprecated and frozen.
